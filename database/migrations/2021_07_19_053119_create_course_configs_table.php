...
class CreateCourseConfigsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('course_configs', function (Blueprint $table) {
            ...
            $table->boolean('ethnic')->default(false)->comment("1 => in ethnic group, 2 => not in ethnic group");
            $table->boolean('freedom_fighter')->default(false);
            $table->boolean('disable_status')->default(false)->comment('1 => disable  2 => not disable');
            $table->boolean('ssc')->default(false)->comment('is passed ssc');
            $table->boolean('hsc')->default(false)->comment('is passed hsc');
            $table->boolean('honors')->default(false)->comment('is passed honors');
            $table->boolean('masters')->default(false)->comment('is passed masters');
            $table->boolean('occupation')->default(false)->comment('is occupation needed');
            $table->boolean('guardian')->default(false)->comment('is guardian information needed');
